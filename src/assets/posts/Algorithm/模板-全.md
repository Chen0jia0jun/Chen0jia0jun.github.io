...
单调队列不常作为一个裸的知识点来单独考，而是**常常与动态规划等问题结合在一起，用作优化时间复杂度**

**特点**（或者说是要求）：

- 在一个区间 [*l*,*r*] 上求最值；
- **区间左右端点 l,r 均单调不减/单调不增**。
- 若求解最大值，则队列是单调不增的（即front部分元素为最大值）；若求解最小值，则队列是单调不减的（即front部分元素为最小值）；
...
```
queue<node> q; //创建队列，可以放进bfs函数里面
void bfs(){
	while(!q.empty())
	{
		node temp=q.front(); //提取出根节点
		q.pop(); //弹出节点
		if(完成目标)
		{
			记录;
		...
		//扩展
		for(int i=0;i<m;i++)//m为方向数组长度，比如遍历上下左右四个方向，则m = 4
		{
			int xx=temp.x+dx[i];
			int yy=temp.y+dy[i];
			if(下标越界||已被标记)
...
https://www.acwing.com/file_system/file/content/whole/index/content/3190/
https://www.acwing.com/file_system/file/content/whole/index/content/11052478/
